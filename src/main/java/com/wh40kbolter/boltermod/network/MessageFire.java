...
public class MessageFire implements IMessage {
    ...
    public MessageFire(EnumHand hand) {
        this.hand = hand;
    }
    ...
    public static class Handler implements IMessageHandler<MessageFire, IMessage> {
        ...
        @Override
        public IMessage onMessage(MessageFire message, MessageContext ctx) {
            EntityPlayerMP player = ctx.getServerHandler().player;
            player.getServerWorld().addScheduledTask(() -> {
                ItemStack stack = player.getHeldItem(message.hand);
                if (!stack.isEmpty() && stack.getItem() instanceof ItemBolter) {
                    ItemBolter.tryFire(player, message.hand);
                }
            });
            return null;
        }
